...
def read_all_to_sequence_records_bundle(directory: str) -> Generator[SequenceRecordsBundle]:
    for file in os.listdir(directory):
        for sequence_records_bundels in __read_to_sequence_records_bundles(f"{directory}/{file}"):
            for sequence_record_bundle in sequence_records_bundels:
    ...
    for taxonomy_id, gene_ids in taxonomy_to_gene_ids_dict.items():
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for genome_id in genome_ids:
                ...
                if gff_file is None: raise ValueError(f"gff file not found for {taxonomy_id}, {genome_id} in {tmp_dir}")
